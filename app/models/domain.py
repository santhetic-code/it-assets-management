...
class Asset(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    asset_tag = Column(String(50), unique=True, index=True, nullable=False) # Harus unik
    ...
    health_reports = relationship("HealthMonitoring", back_populates="asset", cascade="all, delete-orphan")
    purchase_info = relationship("Purchase", back_populates="asset", uselist=False, cascade="all, delete-orphan")
...
class NetworkIP(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    ip_address = Column(String(50), unique=True, index=True, nullable=False) # Harus unik
...
class Credential(Base):
    ...
    password_hash = Column(Text, nullable=False) # Disimpan dalam bentuk enkripsi Fernet
...
class Purchase(Base):
    ...
    total_price = Column(Float, default=0.0) # Nanti akan dihitung otomatis di Service layer
...
class SystemLogs(Base):
    ...
    action = Column(String(255), nullable=False) # Cth: "Mengubah Aset", "Reveal Password"
