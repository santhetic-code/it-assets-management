...
# ---------------------------------------------------------
# 1. SETUP KEAMANAN & KRIPTOGRAFI (Bebas Hardcode)
# ---------------------------------------------------------
...
def verify_jwt_token(token: str) -> dict:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Sesi login telah kedaluwarsa.")
    except jwt.InvalidTokenError:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Token otentikasi tidak valid.")
...
async def verify_csrf_token(request: Request):
    ...
    # Ambil dari form-data (untuk HTML native form tanpa JS)
    form_csrf = None
    ...
    if not cookie_csrf or not client_csrf or cookie_csrf != client_csrf:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Akses Ditolak: Potensi CSRF!")
    return True


# ---------------------------------------------------------
# 5. UPLOAD FILE AMAN (Perbaikan Isu Limitasi Ukuran)
# ---------------------------------------------------------
async def secure_save_file(file: UploadFile, destination_path: str, max_size_mb: int = 5):
    """
    Menyimpan file ke disk secara aman. Memvalidasi ekstensi dan membatasi ukuran.
    """
    ...
    with open(destination_path, "wb") as buffer:
        while True:
            ...
            # Jika ukuran file melebih batas, batalkan dan hapus file sementara
            if file_size > (max_size_mb * 1024 * 1024):
                buffer.close()
                os.remove(destination_path)
                raise HTTPException(status_code=413, detail=f"Gagal: Ukuran file melebihi batas {max_size_mb} MB.")
