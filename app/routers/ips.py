...
@router.post("/", response_model=NetworkIPResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_staff_or_admin), Depends(get_audit_logger), Depends(verify_csrf_token)])
...
@router.put("/{ip_id}", response_model=NetworkIPResponse, dependencies=[Depends(require_staff_or_admin), Depends(get_audit_logger), Depends(verify_csrf_token)])
...
@router.delete("/{ip_id}", dependencies=[Depends(require_staff_or_admin), Depends(get_audit_logger), Depends(verify_csrf_token)])
